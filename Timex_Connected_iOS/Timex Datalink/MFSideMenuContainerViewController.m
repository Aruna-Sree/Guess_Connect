...
@property (retain, nonatomic) MFOpacityView *opacityView; // Added for more visibility stuff
@end

@implementation MFSideMenuContainerViewController
{
    UIPanGestureRecognizer *centarlPan;
    UIPanGestureRecognizer *sideMenuPan;
...
- (void)setDefaultSettings
{
    ...
    self.menuWidth = M328_SIDE_MENU_WIDTH;
    ...
    [titleBarAttributes setValue:[UIFont fontWithName: [iDevicesUtil getAppWideFontName] size: APP_HEADER_FONT_SIZE] forKey: NSFontAttributeName];
    [titleBarAttributes setValue: UIColorFromRGB(COLOR_DEFAULT_TIMEX_WHITE) forKey: NSForegroundColorAttributeName];
...
- (BOOL)shouldAutorotate
{
    if (self.centerViewController)
    {
        ...
        UINavigationController *currentCentertController = [navigationController.viewControllers objectAtIndex: [navigationController.viewControllers count] - 1];
        return [currentCentertController shouldAutorotate];
    }
...
- (NSUInteger)supportedInterfaceOrientations
{
    if (self.centerViewController)
    {
        UINavigationController *navigationController  = self.centerViewController;
        UINavigationController *currentCentertController = [navigationController.viewControllers objectAtIndex: [navigationController.viewControllers count] - 1];
        return [currentCentertController supportedInterfaceOrientations];
    }
...
- (void)setCenterViewController:(UIViewController *)centerViewController
{
    ...
    //Mantis 811 fix
    [centerViewController.view setFrame: self.view.bounds];
}
...
- (void)addGestureRecognizers {
    ...
    centarlPan = [self panGestureRecognizer];
    [[self.centerViewController view] addGestureRecognizer:centarlPan];
    
    sideMenuPan = [self panGestureRecognizer];
    [menuContainerView addGestureRecognizer:sideMenuPan];
}
...
- (void)openLeftSideMenuCompletion:(void (^)(void))completion
{
    ...
    CGRect opacityFrame  = self.opacityView.frame;
    self.opacityView.frame = CGRectMake(0, opacityFrame.origin.y, opacityFrame.size.width, opacityFrame.size.height);
    self.opacityView.userInteractionEnabled = YES;
    [[self.centerViewController view]addSubview:self.opacityView];
}
...
//    {
//        if([gestureRecognizer.view isEqual:[self.centerViewController view]] && [self centerViewControllerPanEnabled] && self.menuState != MFSideMenuStateClosed)
//            return YES;
//        
...
{
    if (gestureRecognizer == sideMenuPan)
    {
        if (centarlPan.state == UIGestureRecognizerStateEnded ||
            centarlPan.state == UIGestureRecognizerStateCancelled ||
            centarlPan.state == UIGestureRecognizerStateFailed ||
            centarlPan.state == UIGestureRecognizerStatePossible)
        {
            return YES;
        }
    }
    else if(gestureRecognizer == centarlPan)
    {
        if (sideMenuPan.state == UIGestureRecognizerStateEnded ||
            sideMenuPan.state == UIGestureRecognizerStateCancelled ||
            sideMenuPan.state == UIGestureRecognizerStateFailed ||
            sideMenuPan.state == UIGestureRecognizerStatePossible)
        {
            return YES;
        }
    }
                           ...
                           completion:(void (^)(void))completion
{
    ...
    if(animated)
    {
        ...
        __weak MFSideMenuContainerViewController * weakSelf = self;
        
        [UIView animateWithDuration:duration animations:^{
            MFSideMenuContainerViewController * strongSelf = weakSelf;
            [strongSelf setCenterViewControllerOffset:offset];
