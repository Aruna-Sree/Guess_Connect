...
NSString * const NXOAuth1ClientConnectionContextTempTokenRequest = @"oauth1tempTokenRequest";
NSString * const NXOAuth1ClientConnectionContextTokenRequest = @"oauth1tokenRequest";
...
@synthesize temporaryAccessToken, temporaryAccessSecret;
...
- (void)setTemporaryAccessToken:(NSString *)value
{
    temporaryAccessToken = value;
    
    if (temporaryAccessToken)
    {
        if ([delegate respondsToSelector:@selector(oauthClientDidGetTemporaryAccessToken:)])
        {
            [delegate oauthClientDidGetTemporaryAccessToken: self];
        }
    }
}
...
- (BOOL)openRedirectURL:(NSURL *)URL redirect: (NSURL *)redirectURL
{
...
- (BOOL)openRedirectURL:(NSURL *)URL includeRedirect:(BOOL)isIncluded
{
    NSString *accessGrant = [URL nxoauth2_valueForQueryParameterKey:@"code"];
    if (accessGrant) {
        if(isIncluded == YES){
        [self requestTokenWithAuthGrant:accessGrant redirectURL:[URL nxoauth2_URLWithoutQueryString]];
             ...
             [self requestTokenWithAuthGrant:accessGrant redirectURL:nil];
        }
        return YES;
    }
...
//                                       [redirectURL absoluteString], @"redirect_uri",
                                       ...
                                       nil];
    if(redirectURL != nil){
        [parameters setObject:[redirectURL absoluteString] forKey:@"redirect_uri"];
    }
...
- (void)requestOAuth1TemporaryTokenFromURL:(NSURL *)URL;
{
//    NSAssert1(!authConnection, @"authConnection already running with: %@", authConnection);
    ...
    authConnection.context = NXOAuth1ClientConnectionContextTempTokenRequest;
}

- (void)requestOAuth1TokenFromURL:(NSURL *)URL;
{    
    [authConnection cancel];  // just to be sure
    ...
    authConnection.context = NXOAuth1ClientConnectionContextTokenRequest;
}
...
- (void)refreshAccessTokenWithURL: (NSString *) url
{
...
- (void)oauthConnection:(NXOAuth2Connection *)connection didFinishWithData:(NSData *)data;
{
    if (connection == authConnection)
    {
        ...
        if (authConnection.context == NXOAuth1ClientConnectionContextTempTokenRequest)
        {
            //we didn't get the proper token, all we got is a temp token that we need to save...
            //and the start the dance again with authorization, etc.
            NSArray * bits = [result componentsSeparatedByString: @"&"];
            NSArray * token_bits = [[bits objectAtIndex: 0] componentsSeparatedByString: @"="];
            NSArray * secret_bits = [[bits objectAtIndex: 1] componentsSeparatedByString: @"="];
            
            //read the secret first! reading the token triggers the callback to the delegate
            self.temporaryAccessSecret = [secret_bits objectAtIndex: 1];
            
            self.temporaryAccessToken = [token_bits objectAtIndex: 1];
        }
        else if (authConnection.context == NXOAuth1ClientConnectionContextTokenRequest)
        {            
            NSArray * bits = [result componentsSeparatedByString: @"&"];
            NSArray * token_bits = [[bits objectAtIndex: 0] componentsSeparatedByString: @"="];
            NSArray * secret_bits = [[bits objectAtIndex: 1] componentsSeparatedByString: @"="];
            
            NXOAuth2AccessToken *newToken = [[NXOAuth2AccessToken alloc ] initWithAccessToken:[token_bits objectAtIndex: 1] andSecretToken: [secret_bits objectAtIndex: 1]];
