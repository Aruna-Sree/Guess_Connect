...
const CGFloat           kSDSegmentedControlArrowSize = 0;
const CGFloat           kSDSegmentedControlInterItemSpace = 30.0;
const UIEdgeInsets      kSDSegmentedControlStainEdgeInsets = {-2, -16, -4, -16};
const CGSize            kSDSegmentedControlImageSize = {WORKOUT_DETAILS_NAVIGATION_ICON_SIZE, WORKOUT_DETAILS_NAVIGATION_ICON_SIZE};
...
{
    __weak SDSegmentedControl * weakSelf = self;
    if ((self = [self init]))
    {
        [items enumerateObjectsUsingBlock:^(id title, NSUInteger idx, BOOL *stop)
        {
            SDSegmentedControl * strongSelf = weakSelf;
            [strongSelf insertSegmentWithTitle:title atIndex:idx animated:NO];
        }];
    }
    return self;
}
...
- (void)commonInit
{
    ...
    frame.size.height = WORKOUT_DETAILS_NAVIGATION_ICON_SIZE + (WORKOUT_DETAILS_NAVIGATION_ICON_SPACE_OFFSET * 2 + SD_SEGMENTED_CONTROL_FONT_HEIGHT);
    self.frame = frame;
...
- (void)insertSegmentWithImage:(UIImage *)image selectedImage: (UIImage *)imageSelected title: (NSString *) titleText atIndex:(NSUInteger)index animated:(BOOL)animated
{
    [self insertSegmentWithTitle: titleText image:image selectedImage: imageSelected atIndex:index animated:animated];
}
...
- (void)setImage:(UIImage *)image andSelectedImage:(UIImage *)imageSelected forSegmentAtIndex:(NSUInteger)index
{
    SDSegmentView* segmentView = [self segmentAtIndex:index];
    [segmentView setImage:image forState:UIControlStateNormal];
    [segmentView setImage:imageSelected forState:UIControlStateSelected];
...
{
    [self insertSegmentWithTitle:title image:nil selectedImage: nil atIndex:index animated:animated];
}
...
- (void)insertSegmentWithTitle:(NSString *)title image:(UIImage *)image selectedImage: (UIImage *)imageSelected atIndex:(NSUInteger)index animated:(BOOL)animated
{
    ...
    if (image == nil && imageSelected == nil)
    {
        [segmentView setTitle:title forState:UIControlStateNormal];
        
        //this is just a text-based button, so we can just set it and forget it
        [segmentView sizeToFit];
    }
    else
    {
        ...
        [segmentView adjustImageVsTitle];
    }
...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
    ...
    {
        frame.size.height = kSDSegmentedControlImageSize.height;
    }
...
- (void)layoutSegments
{
    ...
    BOOL animated = ignoreStain ? self.animationDuration : self.animationDuration && !CGRectEqualToRect(self._selectedStainView.frame, CGRectZero);
    ...
    else
    {
        ...
        [UIView animateWithDuration:animated ? self.animationDuration : 0 animations:^
        {
            SDSegmentedControl * strongSelf = weakSelf;
            if (!ignoreStain)
                strongSelf._selectedStainView.frame = stainFrame;
        }
        completion:^(BOOL finished)
        {
            SDSegmentedControl * strongSelf = weakSelf;
            for (SDSegmentView *item in strongSelf._items)
            {
...
- (void)drawPathsFromPosition:(CGFloat)oldPosition toPosition:(CGFloat)position animationDuration:(CFTimeInterval)duration completion:(void (^)(void))completion
{
    ...
    //do not draw shadow mask for Timex
    /*
...
//    [appearance setTitleShadowColor:UIColor.whiteColor forState:UIControlStateNormal];
    ...
    [appearance setTitleColor:[UIColor blackColor] forState:UIControlStateSelected];
//    [appearance setTitleShadowColor:UIColor.whiteColor forState:UIControlStateSelected];

    [appearance setTitleColor:[UIColor lightGrayColor] forState:UIControlStateDisabled];
//    [appearance setTitleShadowColor:UIColor.clearColor forState:UIControlStateDisabled];

    [appearance setItemFont:[UIFont fontWithName: [iDevicesUtil getAppWideFontName] size: RANGE_SEGMENTS_FONT_SIZE]];
...
- (id)initWithFrame:(CGRect)frame
{
    if ((self = [super initWithFrame:frame]))
    {
        ...
        self.contentEdgeInsets = UIEdgeInsetsMake(0, 0.0, 0, 0.0); // Enlarge touchable area
...
- (void)adjustImageVsTitle
{
    ...
    self.imageEdgeInsets = UIEdgeInsetsMake(-(SD_SEGMENTED_CONTROL_FONT_HEIGHT + WORKOUT_DETAILS_NAVIGATION_ICON_SPACE_OFFSET), 0, 0.0, 0);
}
